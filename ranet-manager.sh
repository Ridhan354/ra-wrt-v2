...
ensure_init_script() {
    ...
    if [ -f "$INIT_SCRIPT" ]; then
        if grep -q "python3 tidak ditemukan" "$INIT_SCRIPT" 2>/dev/null \
            && grep -q "start()" "$INIT_SCRIPT" 2>/dev/null; then
            return 0
        fi
        echo "Init script lama ditemukan. Memperbarui..."
        need_update=1
    fi
...
INIT
    ...
    if [ "$need_update" -eq 1 ]; then
        echo "Init script diperbarui di $INIT_SCRIPT"
    else
